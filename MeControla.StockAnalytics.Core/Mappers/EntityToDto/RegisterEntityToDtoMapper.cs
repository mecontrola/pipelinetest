...
namespace MeControla.StockAnalytics.Core.Mappers.EntityToDto
{
    ...
    internal
#endif
    interface IRegisterEntityToDtoMapper : IEntityToDtoMapper<Register, RegisterDto>
    { }
    ...
    internal
#endif
    sealed class RegisterEntityToDtoMapper : BaseMapper<Register, RegisterDto>, IRegisterEntityToDtoMapper
    {
        protected override void MapFields(IMappingExpression<Register, RegisterDto> map)
            => map.ForMember(dest => dest.Id, opt => opt.MapFrom(source => source.Uuid))
                  .ForMember(dest => dest.Name, opt => opt.MapFrom(source => source.Name))
                  .ForMember(dest => dest.Website, opt => opt.MapFrom(source => source.Website))
